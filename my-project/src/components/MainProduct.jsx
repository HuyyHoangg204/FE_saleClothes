...
import { filterListProductByCategory, getListProductByCategory } from '../redux/apiRequest';
import { useLocation, useParams } from 'react-router-dom';
import { categories } from '../pages/sale/categoryData';

function MainProduct({ title, dataFilter }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            if (!dataFilter.filter) {
                try {
                    const res = await getListProductByCategory(matchedCategory.id, currentPage, 28, isSort);
                    setData(res);
                    setLoading(false);
                } catch (error) {
                    console.log(error);
                ...
                try {
                    console.log(dataFilter?.size);

                    const res = await filterListProductByCategory(
                        matchedCategory.id,
                        currentPage,
                        28,
                        isSort,
        ...
        fetchData();
    }, [categorySlug, currentPage, isSort, dataFilter]);
        ...
        <div className="w-full ml-12">
            <div className="flex justify-between items-center">
                ...
                >
                    ...
                    <img
                        className={`w-9 h-9 transition-transform duration-300 ${isIconRotated ? 'rotate-180' : ''}`}
                        src={icons.iconDropDownArrow}
                        alt=""
                    />
                </div>
                {showSortProduct && <SortProduct handleSortProduct={handleSortProduct} />}
            </div>
            {haveProduct ? (
                <div>
